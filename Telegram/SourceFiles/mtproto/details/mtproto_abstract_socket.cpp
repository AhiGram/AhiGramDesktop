...
// AhiGram include
#include "ahigram/networking/ahi_socket_wrap.h"

namespace MTP::details {

std::unique_ptr<AbstractSocket>
AbstractSocket::Create(not_null<QThread *> thread, const bytes::vector &secret,
                       const QNetworkProxy &proxy, bool protocolForFiles) {
  auto result = [&]() -> std::unique_ptr<AbstractSocket> {
  ...
  // ahigram mtproto fragmentation bypass
  return AhiGram::Networking::Wrap(std::move(result), thread, protocolForFiles);
}
